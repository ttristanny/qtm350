...
SELECT
    colours.colour_name,
    sizes.size_code,
    colours.colour_name || ' - ' || sizes.size_code as t_shirt -- Use || for concatenation
FROM colours
CROSS JOIN sizes
...
- Let's find all players who have either scored more than 5 goals OR achieved more than 3 victories. `UNION` is suitable here because we are combining two subsets of the *same* entity type (players) based on different criteria, and we want a unique list.
...
:::{style="margin-top: 50px; font-size: 23px;"}
- You can also drop the view if you no longer need it.

```{python}
#| echo: true
#| eval: true
# Drop the view if it exists
cursor.execute('DROP VIEW IF EXISTS player_team_stats;')
connection.commit() # Commit changes
print("View 'player_team_stats' dropped.")
```
:::
